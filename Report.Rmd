...
#isocrhone
o_iso <- st_transform(filter(.data = stp, CODINFRA == top_odline$par_subida), 4326) %>% mb_isochrone(profile = "walking", time = 5)
# map
...
lft %>%
  addPolygons(data = o_iso,
              popup = "isocrona de 5 min. (caminata)") %>%
  addFlows(fl_lt$lng0,
           fl_lt$lat0,
           fl_lt$lng1,
           fl_lt$lat1,
...
&nbsp;&nbsp;En el cuadro a continuación se visualizan los principales indicadores del **servicio-sentido-periodo**
...
<br></br>
<center>En el siguiente gráfico se muestran todos los sube-baja del periodo para tener una completa visualización del uso del *servicio-sentido*. </center>
<br></br>
...
<br></br>
La siguiente tabla permite identificar las paradas que componen el sube-baja del servicio.
<br></br>
...
buff_z <- lapply(X = 1:nrow(od_xy),
       FUN = function(z) st_buffer(x = filter(.data = stp,
                                              CODINFRA %in% c(od_xy$par_subida[z], od_xy$par_bajada[z])
                                              ),
                                   dist = 300
                                   )
       )
inter_aa <- lapply(X = buff_z,
                   FUN = function(aa) select(.data = st_drop_geometry(st_join(x = aa,
                                                                              y = stp,
                                                                              join = st_intersects
                                                                              )
                                                                      ),
...
comb_ac <- unique(unlist(lapply(X = inter_ab,
                                FUN = function(ab) as.vector(outer(ab[[2]]$CODINFRA.y,
                                                                   ab[[1]]$CODINFRA.y,
                                                                   paste,
                                                                   sep = "|"
                                                                   )
...
od_xy_spp <- nrow(filter(.data = od_sersen,
                                PER_DTPM2 == y & (parOD %in% comb_ac)
                         )
                  )/nrow(od_xy)
```

**`r paste("El índice de superposición para el Servicio Sentido", x, "en el periodo", y, "es:", od_xy_spp, sep = " ")`**
